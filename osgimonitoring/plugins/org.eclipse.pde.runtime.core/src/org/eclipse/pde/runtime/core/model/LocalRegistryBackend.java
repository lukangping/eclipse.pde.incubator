...
import org.eclipse.pde.runtime.core.MessageHelper;
import org.eclipse.pde.runtime.core.PDERuntimeCorePlugin;
...
public class LocalRegistryBackend implements IRegistryEventListener, BundleListener, ServiceListener, RegistryBackend {
	...
	public boolean connect(IProgressMonitor monitor) {
		...
		PDERuntimeCorePlugin.getDefault().getBundleContext().addBundleListener(this);
		Platform.getExtensionRegistry().addListener(this);
		PDERuntimeCorePlugin.getDefault().getBundleContext().addServiceListener(this);
	...
	public void disconnect() {
		...
		PDERuntimeCorePlugin.getDefault().getBundleContext().removeBundleListener(this);
		PDERuntimeCorePlugin.getDefault().getBundleContext().removeServiceListener(this);
	}
	...
	public void start(long id) {
		try {
			PDERuntimeCorePlugin.getDefault().getBundleContext().getBundle(id).start();
	...
	public void stop(long id) {
		try {
			PDERuntimeCorePlugin.getDefault().getBundleContext().getBundle(id).stop();
		} catch (BundleException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public String[] diagnose(long id) {
		PlatformAdmin plaformAdmin = PDERuntimeCorePlugin.getDefault().getPlatformAdmin();
		State state = plaformAdmin.getState(false);

		BundleDescription desc = state.getBundle(id);

		PlatformAdmin platformAdmin = PDERuntimeCorePlugin.getDefault().getPlatformAdmin();
	...
	public void initializeBundles(IProgressMonitor monitor) {
		...
		org.osgi.framework.Bundle[] newBundles = PDERuntimeCorePlugin.getDefault().getBundleContext().getBundles();
	...
	public void initializeServices(IProgressMonitor monitor) {
		...
		try {
			references = PDERuntimeCorePlugin.getDefault().getBundleContext().getAllServiceReferences(null, null);
	...
	private static Long getBundleId(String name) {
		BundleDescription descr = PDERuntimeCorePlugin.getDefault().getPlatformAdmin().getState().getBundle(name, null);
	...
	public void setEnabled(long id, boolean enabled) {
		State state = PDERuntimeCorePlugin.getDefault().getState();
		BundleDescription desc = state.getBundle(id);
		...
		org.osgi.framework.Bundle b = PDERuntimeCorePlugin.getDefault().getBundleContext().getBundle(id);
		PackageAdmin packageAdmin = PDERuntimeCorePlugin.getDefault().getPackageAdmin();
